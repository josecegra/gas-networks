qf   = x_t(1:sum(Nv));
pf   = x_t(sum(Nv)+1:2*sum(Nv));
for j = 1:w_pipes
    q_wp(j) = x_t(sN(size(sN,1))+j);
    p_wp(2*j-1) = x_t(sN(size(sN,1))+w_pipes + 2*j -1);
    p_wp(2*j) = x_t(sN(size(sN,1))+w_pipes + 2*j); 
...
subplot(2,1,1);
plot(h*(0:(sum(N_vv)-1)),p_p-p0)
subplot(2,1,2);
plot(h*(0:(sum(N_vv)-1)),q_p-q0)
